...
// table, declared NOT NULL and written ” for the default axis, and that the
// composite foreign keys carry it: the same key order as the Spanner store, so
// a single-axis application's rows stay correct unchanged when it later
// declares an axis.
func TestDDL_axisKey(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "roles", stmt: 0, want: []string{`"Axis" TEXT NOT NULL`, `PRIMARY KEY ("IsGlobal", "Axis", "Domain", "Role")`}},
		{name: "user roles", stmt: 1, want: []string{
			`"Axis" TEXT NOT NULL`,
			`PRIMARY KEY ("IsGlobal", "Axis", "Domain", "Role", "User")`,
			`FOREIGN KEY ("IsGlobal", "Axis", "Domain", "Role") REFERENCES "AccessRoles" ("IsGlobal", "Axis", "Domain", "Role")`,
		}},
		{name: "user roles by scope and user", stmt: 2, want: []string{`ON "AccessUserRoles" ("IsGlobal", "Axis", "Domain", "User")`}},
		{name: "role grants", stmt: 3, want: []string{
			`"Axis" TEXT NOT NULL`,
			`PRIMARY KEY ("IsGlobal", "Axis", "Domain", "Role", "Permission", "Resource", "Field", "Condition")`,
			`FOREIGN KEY ("IsGlobal", "Axis", "Domain", "Role") REFERENCES "AccessRoles" ("IsGlobal", "Axis", "Domain", "Role") ON DELETE CASCADE`,
		}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
